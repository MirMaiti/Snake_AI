...
class Snake(object):
    ...
    def vision(self,fruit):
        ...
        # max_distance = math.sqrt((num_rows - 1) ** 2 + (num_columns - 1) ** 2)
        vision = []
        ...
        for dx, dy in directions:
            ...
            while ok(x,y):
                ...
                # Check if the vision ray hits the snake's body
                if (x, y) in [segment.pos for segment in self.body]:
                    break

                # Check if the vision ray hits the snack
                if (x, y) == fruit.pos:
                    break
            
            # norm_distance = normalize(distance,0,max_distance)
